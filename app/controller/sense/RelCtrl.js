		...
		function(
			...
			$scope.getRelation(relId).then(function(rel) {
				...
				if ($scope.tempRel) {
					...
					if ($scope.tempRel.a_sense == $scope.currentSense.id) {
						...
						$scope.relation.targetSense = wnwbApi.Sense.get({
							id : $scope.tempRel.b_sense
						});
					}
					if ($scope.tempRel.b_sense == $scope.currentSense.id) {
						$scope.relation.direction = 'ba';
						$scope.relation.targetSense = wnwbApi.Sense.get({
							id : $scope.tempRel.a_sense
						});
					}
				} else {
					$scope.tempRel = {};
			...
			$scope.selectTarget = function() {
				return $uibModal.open({
					...
					if (sense) {
						var targetSense = wnwbApi.Sense.get({
							id : sense.id
						}, function() {
				...
				if (targetSense) {
					...
					if (relType) {
						if ($scope.relation.direction == 'ab') {
							$scope.setRel({
								id : $scope.currentSense.id,
								label : $scope.currentSense.label
							}, {
								id : targetSense.id,
								label : targetSense.label
							}, relType.id);
			...
			$scope.saveRelPromise = function() {
				...
				if (!$scope.validateRel($scope.tempRel)) {
					fValid = false;
				}
