		...
		function(
			...
			$scope.getRelation(relId).then(function(rel) {
				...
				if ($scope.tempRel) {
					...
					if ($scope.tempRel.a_synset == $scope.currentSynSet.id) {
						...
						$scope.relation.targetSynSet = wnwbApi.SynSet.get({
							id : $scope.tempRel.b_synset
						});
					}
					if ($scope.tempRel.b_synset == $scope.currentSynSet.id) {
						$scope.relation.direction = 'ba';
						$scope.relation.targetSynSet = wnwbApi.SynSet.get({
							id : $scope.tempRel.a_synset
						});
					}
				} else {
					$scope.tempRel = {};
			...
			$scope.selectTarget = function() {
				return $uibModal.open({
					...
					if (synset) {
						var targetSynset = wnwbApi.SynSet.get({
							id : synset.id
						}, function() {
					...
					if (relType) {
						if ($scope.relation.direction == 'ab') {
							$scope.setRel({
								...
								label : $scope.currentSynSet.label
							}, {
								id : targetSynSet.id,
