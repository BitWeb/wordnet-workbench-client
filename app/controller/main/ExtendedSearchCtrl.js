		...
		var initSearchParams = function(hash, searchtype) {
			...
			for (el in parameters) {
				if (parameters[el].field) {
					...
					} else if (parameters[el].values.trim().startsWith('[') && parameters[el].values.trim().endsWith(']')) {
						setname = parameters[el].values.trim().slice(1, -1);
						if (hash[setname]) parameters[el].fixedValues = hash[setname];
